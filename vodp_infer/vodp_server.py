...
def _build_model(cfg: Dict[str, Any]):
    ...
    logger.info(f"Building model with config:")
...
class VODPService:
    def __init__(self, model):
        self.model = model
        self.infer_count = 0
        logger.info("VODPService initialized")
...
class ModelServer:
    def __init__(self, host: str, port: int, service: VODPService):
        self.host = host
        self.port = port
    ...
    def start(self) -> None:
        logger.info(f"Starting VODP server on {self.host}:{self.port}")
        self.sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
        self.sock.bind((self.host, self.port))
        self.sock.listen(5)
        self.running = True
        logger.info(f"VODP server listening on {self.host}:{self.port}")
...
def main() -> None:
    logger.info("=" * 60)
    logger.info("Starting VODP Server")
    logger.info("=" * 60)
    
    parser = argparse.ArgumentParser(description="VODP Inference Server")
    ...
    port = int(server_cfg.get("port", 8456))
    
    logger.info(f"Server configuration: host={host}, port={port}")

    model = _build_model(cfg)
    service = VODPService(model)
