...
def _build_model_obs(
    ...
    """
    根据配置的 model_obs_map，直接从 datacenter 获取数据并构建模型期望的观察格式
    """
    ...
    # 获取模型观察映射配置：模型期望键名 -> datacenter数据源
    model_obs_map = mapping.get("model_obs_map", {})
    # 构建嵌套字典结构：observation.images.image -> {"observation": {"images": {"image": ...}}}
    model_obs = {}
    ...
    for model_key, dc_source in model_obs_map.items():
        if model_key.startswith("observation.images."):
            ...
            # 将点分隔键转换为嵌套结构：observation.images.image -> observation.images.image
            # 但需要组织成嵌套字典
...
def main() -> None:
    ...
    # 从合并后的配置中提取 datacenter 配置并创建配置对象
    ...
    try:
        ...
        client = ModelClient(
            ...
            port=int(client_cfg.get("server_port", 9010)),
        ...
        for step_idx in range(max_steps):
            ...
            for action_h in action_seq:
                dc_action = _map_action_to_datacenter(action_h, center.config.arms, mapping)
                
                print("即将发布动作:", np.array2string(dc_action, max_line_width=200, separator=", "))
                #input("请确认是否发布动作")
                center.publish_action(dc_action)
                
                time.sleep(1.0 / send_freq if send_freq > 0 else 0)

            print(f"step {step_idx + 1}/{max_steps} action_dim={action_seq.shape[-1]}")
